   ...
   tam_pacote = atoi(argv[3]);
	...
	while(1){
		if (nb_clients != 10){
			...
			if(client_fd < 0){
				perror("\nERRO: aceitando conexão do cliente\n");
			}
		 	nb_clients++;
	...
	printf("\nERRO: numero de parametros incorreto\n");
	printf("\nsintaxe: server <banda> <porta> <tam_pacote>\n");
	return (-1);
}
}
void* client_handler(void *client_data){
	...
	if (bytes_read < 0){
		...
		if(!strcmp(buffer, "toto.avi")){
			...
			while(tmp < filesize){
				...
				//bytes_read = fread(out_buffer, tam_pacote, 1, arq);
				bytes_read = read(fd, out_buffer, tam_pacote);
				if (bytes_read < 0){
