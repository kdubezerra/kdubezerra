...
\begin{itemize}
  ...
  \item \textit{am\_multicast}, with the messages ready to be \amdel{}ed, but which may be waiting for barriers from the groups in $sendersTo(G)$;
  \item \textit{am\_delivered}, which contains the messages that have been \amdel{}ed already.
\end{itemize} 
...
\INDENT{Initialization}
  \STATE $k \leftarrow 0$, $nextProp \leftarrow 0$, $local\_decided \leftarrow \emptyset$, $am\_delivered \leftarrow \emptyset$, $local\_msgs \leftarrow \emptyset$, $am\_multicast \leftarrow \emptyset$
...
\WHEN{\rmdelarg{$m$}}
  ...
  \ELSIF{$m \notin am\_multicast$} \label{algline:notbptwice}
    \STATE $am\_multicast \leftarrow am\_multicast \cup \{m\}$ \label{algline:insbp1}
...
\WHEN{$local\_msgs \setminus local\_decided \neq \emptyset$}
  \STATE $k \leftarrow k + 1$
	\STATE $undecided \leftarrow local\_msgs \setminus local\_decided$ \label{algline:undecided}
	...
	\WHILE{$msgSet \setminus local\_decided \neq \emptyset$}
		...
		\IF{$g \in m.dst$} \label{algline:checkcons}
			\STATE $am\_multicast \leftarrow am\_multicast \cup \{m\}$  \label{algline:insbp2}
		\ENDIF
...
\blankline
%\WHEN{$\exists m \in am\_multicast : \forall G' \in sendersTo(G): m.ts < barrier(G',G)$\\ $\wedge\ \nexists m' \in am\_multicast : m'.ts < m.ts$} \label{algline:inam\_multicast}
\WHEN{$am\_multicast \setminus am\_delivered \neq \emptyset$} \label{algline:inammulticast}
	\STATE let $m$ be the message in $am\_multicast \setminus am\_delivered$ with smallest timestamp \label{algline:condel1}
	\IF{$\forall h \in sendersTo(g): m.ts < barrier(h)$} \label{algline:condel2}
%		\STATE $am\_multicast \leftarrow am\_multicast \setminus \{m\}$ \label{algline:rmfrombp}
		\IF {$m \neq null$}
		  \STATE \amdelarg{$m$} \label{algline:consdeliver}
		\ENDIF
		\STATE $am\_delivered \leftarrow am\_delivered \cup \{m\}$ \label{algline:addtodelivered}
	\ENDIF
\ENDWHEN
\blankline
...
\INDENT{\textit{When} \rmdelarg{$m'$}}
  ...
  \ELSIF{$m' \notin am\_multicast \wedge m' \notin am\_delivered$}
    \STATE $barrier(m'.src,G) \leftarrow m'.ts$ \label{algline:incbar}
    \IF {$G \in m'.dst$}
      \STATE $am\_multicast \leftarrow am\_multicast \cup \{m'\}$
    \ENDIF
...
\blankline
\INDENT{\textit{When $\exists m \in am\_multicast : \forall G' \in sendersTo(G): m.ts < barrier(G',G)$\\ $\wedge\ \nexists m' \in am\_multicast : m'.ts < m.ts$ }}
  \STATE $am\_multicast \leftarrow am\_multicast \setminus \{m\}$
  \IF{$m \neq null$}
    \STATE \consdel{$m$}
  \ENDIF
  \STATE $am\_delivered \leftarrow am\_delivered \cup \{m\}$
...
% \blankline
% \INDENT {Initialization}
%   \STATE $k \leftarrow 0$, $nextProp \leftarrow 0$, $local\_decided \leftarrow \emptyset$, $local\_msgs \leftarrow \emptyset$, $optPending \leftarrow \emptyset$, $am\_multicast \leftarrow \emptyset$
%   \INDENT{\textbf{for all} $G' \in sendersTo(G)$ \textbf{do}}
%     \STATE $barrier(G',G) \leftarrow -\infty$ 
%   \ENDINDENT
% \ENDINDENT 
...
Once every correct processes in a group $g$ has inserted a message $m$ into its $local\_msgs$ set, $m$ will be decided within $g$.
\end{lems}

\begin{proof}
Once each correct process $p$ from $g$ has received $m$ and inserted into its $local\_msgs$ list, $m$ will be proposed by $p$ in every consensus instance. The message $m$ is not proposed anymore by $p$ only once it has been inserted into $local\_decided$ (lines \ref{algline:undecided} and \ref{algline:propose}). Finally, $m$ is inserted into $local\_decided$ only after being decided (l. \ref{algline:addtodecided}). Therefore, every processes of $g$ will propose $m$ at some point and $m$ will eventually be decided.
\end{proof}
...
Once a message $m$ has been \amcast{}, it will be inserted into the $am\_multicast$ list of all the correct processes to which such message has been addressed.
\end{lems}

\begin{proof}
In Algorithm \ref{algorithm:deliveryminimal}, whenever a process in a group $g$ \amcast{}s a message, it is first \rmcast{} to all processes in $g$ (l. \ref{algline:rmlocal}). From the uniform agreement property of the \rmcast{} primitive, every correct process from $g$ \rmdel{}s $m$ and inserts it into its $local\_msgs$ set (l. \ref{algline:addtogroupmsgs}). From Lemma \ref{lemma:mcastdecided}, $m$ will eventually be decided within $g$. At this point, if $g \in m.dst$, every correct process of $g$ will also insert $m$ into its $am\_multicast$ list (l. \ref{algline:insbp2}). Besides, each correct process in $g$ \rmcast{}s $m$ to all other destination groups that $m$ is adressed to.

Let $q$ be any process in a destination group $h$ of the message $m$, such that $h \neq g$. In this case, once $m$ is \rmdel{}ed by $q$, such message is inserted into the $am\_multicast$ list of $q$, unless this has been already done (lines \ref{algline:notbptwice} and \ref{algline:insbp1}).
\end{proof}
...
\begin{lems} \label{lemma:barrierperiodic}
  Given a message $m$ in the $am\_multicast$ list of a correct process $p$ from group $g$, eventually every group $h \in sendersTo(g)$ will have sent $barrier(h) > m.ts$ to $p$.
\end{lems}
...
This means that, at some point, all correct processes of each group in $sendersTo(g)$ will have inserted a message $m'$ -- be it $null$ or not -- with a timestamp greater than or equal to that of $m$ into their respective $local\_msgs$ lists. From Lemma \ref{lemma:mcastdecided}, $p$ will insert $m'$ into its $am\_multicast$ list. From the algorithm, the timestamps of the messages are no longer changeable after they have been inserted into $am\_multicast$ so, eventually, $p$ will have in such list a barrier with a value greater than or equal to $m.ts$ from each group in $sendersTo(g)$.
\end{proof}
...
If a process $p$ from a group $g$ has received a message $m$ from another group $h \neq g$, then $p$ has also received any message $m'$ from $h$, where $m'.ts < m.ts$.
\end{lems}

\begin{proof}
Every process $q$ from $h$ sends messages to the processes of $g$, including $p$, using a FIFO channel, via the \rmcast{} primitive, in the same order in which messages are decided within $h$. Also, the agreement property of consensus ensures that all processes from $h$ will decide the same messages in each consensus instance $k$. Finally, $q$ \rmcast{}s to $g$ every message $m : m.src = h \wedge g \in m.dst$ (l. \ref{algline:rmothers} of Algorithm \ref{algorithm:deliveryminimal}).
...
Once a message has been inserted into the $local\_msgs$ list of a given process, it will always belong to it.
\end{lems}

\begin{proof}
Immediate from the algorithm, considering that a message is never removed from the $local\_msgs$.
\end{proof}
...
\begin{lems} \label{lemma:oncedecidednotsmaller}
  Once a process has inserted a message $m$ from some group into its $am\_multicast$ list, no message $m' : m'.ts < m.ts$ from the same group will be inserted into such $am\_multicast$ list afterwards.
\end{lems}

\begin{proof}
Let $p$ be a process from the group $g$ which sent $m$. Before inserting $m$ into the $local\_decided$ list (l. \ref{algline:addtodecided} of Algorithm \ref{algorithm:deliveryminimal}), $p$ checks whether there is already some other message with an equal or lower timestamp in such list. If that is the case, the timestamp of $m$ is changed to a value greater than that of any other message already within $local\_decided$. Only then $m$ may be inserted into the $am\_multicast$ list of $p$ (l. \ref{algline:insbp2}). Therefore, $p$ inserts messages from other processes of $g$ into its $am\_multicast$ list in ascending order of timestamps. Besides, each process from $g$ \rmcast{}s $m$ to the other destination groups of $m$ in this same order (l. \ref{algline:rmothers}).

Regarding groups other than the source of the message, let $q$ be any process that is a destination of $m$, such that $q \in h \wedge h \neq g$. The process $q$ inserts $m$ into its $am\_multicast$ list in line \ref{algline:insbp1}. Considering that every process in a group sends that group's messages to other groups in the same order in which they are decided, that once a group has received some message from another group, any message prior to that one from the same group has also already been received (from Lemma \ref{lemma:groupfifo}), and that no message is inserted twice in the $ar\_multicast$ list of any process (from line \ref{algline:notbptwice} and Lemma \ref{lemma:alwaysbpdel}), we can infer that, once $q$ inserts $m$ into its $am\_multicast$ list, no message $m' : m'.ts < m.ts$ from $g$ will be inserted into the the $am\_multicast$ list of $q$ afterwards.
\end{proof}
...
Once a message $m$ has been inserted into the $am\_multicast$ list of a process $p$, it will eventually be inserted into $am\_delivered$ and, if $m$ is different from $null$, it will be delivered by $p$.
\end{lems}

\begin{proof}
Eventually, $p$ will have received a barrier from every group in $sendersTo(group(p))$ with a value greater than the timestamp of $m$ (from Lemma \ref{lemma:barrierperiodic}). Once this happens, as any barrier is also a message, no more messages with a timestamp lower than that of $m$ will be inserted into the $am\_multicast$ list of $p$ (from the definition of $sendersTo(group(p))$ and Lemma \ref{lemma:oncedecidednotsmaller}). Let $am\_ready$ be the set of $m$ plus all undelivered messages from $am\_multicast$ whose timestamps are lower than that of $m$, that is, $am\_ready = \{m\} \cup \{m' : m' \in am\_multicast \setminus am\_delivered \wedge m'.ts < m.ts\}$.

We can infer that no more messages will be included in this set, meaning that any other message that might be later inserted into $am\_multicast$ will have a timestamp greater than that of any message in $am\_ready$. Besides, the barrier received for $m$ also applies to all the other messages in this set, since their timestamps are lower than that of $m$. We can, therefore, prove that all these messages will eventually be delivered by induction on the position $i$ of each message $m_i$ in $am\_ready$, in ascending order of timestamps.\\
\\
\noindent Base case ($i=1$): Let $m_1$ be the first message in $am\_ready$. It means that $\nexists m \in am\_multicast \setminus am\_delivered : m.ts < m_1.ts$. The message $m_1$ has the lowest timestamp in the list and all the necessary barriers have been received already. Therefore, $m_1$ satisfies all conditions from lines \ref{algline:condel1} and \ref{algline:condel2} of Algorithm \ref{algorithm:deliveryminimal} and will be removed from $barPending$. If it is different from $null$, it will also be delivered.\\
\\
...
If a message $m$ was delivered to process $p$ of group $G$, then it has (1) been \amcast{} before, (2) $G \in m.dst$ and (3) it has not been \cons{}ed by $p$ before.
\end{props}
...
\begin{proof}
Let $p$ be the process that is \amcast{}ing $m$. We can prove this lemma by demonstrating that any other process $p'$ will agree with $p$ on the timestamp of $m$. Let us first consider the case of $p'$ belonging to the same group as $p$. We can prove by induction on the identifier $k$ of each consensus instance within $group(p)$:\\
...
If a process $p$ \amcast{}s $m$, then $m'$, then the final values of their timestamps will be such that $m.ts < m'.ts$.
\end{lems}

\begin{proof}
From lines \ref{algline:gettime} and \ref{algline:rmlocal}, and the properties of the \rmcast{} primitive, we know that all correct processes \rmdel{} $m$ and then $m'$, which means that each process $p'$ from $group(p)$ also inserts them into its $propPending$ list in this same order.
...
If $p$ \amcast{}s $m$ and then $m'$, then no process $p'$ in both $m.dst$ and $m'.dst$ delivers $m'$ before delivering $m$.
\end{props}
