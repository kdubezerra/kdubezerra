...
Hereafter, we assume that in addition to a set of destination groups, $m.dst$, each message $m$ has a source group, $m.src$, and a timestamp, $m.ts$. To \amcast{} $m$, process $p$ in group $g$ sets $m.ts$ to a unique timestamp based on its real-time local clock and then \rmcast{}s $m$ to all processes in $g$.
...
A more formal description of the protocol is given in Algorithm \ref{algorithm:deliveryminimal}. We use a $getTime()$ primitive, which returns the current value of the local wallclock, which is monotonically increasing. Moreover, each process $p$ of group $g$ keeps some sets of messages:
\begin{itemize}
...
Each consensus instances may decide not only one, but a batch of messages. When a message set is decided, the messages in it are handled in ascending order of timestamps, as if they were decided separately in the original timestamp order. 
