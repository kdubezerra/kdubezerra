...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    In this case, when $m$ is decided, $p$ \rmc{}s $m$ to every \mbox{$p' \in (m.dst \setminus \{G\})$}. When $m$ is \rmd{}ed by each $p' \in G'$, $p'$ checks whether it has ever inserted $m$ in its own $barPending$ list. If not, $p'$ inserts $m$ into $barPending$ and adjusts $barrier(G, G')$ to $m.ts$. As the sending of these messages from a group to another is done via a FIFO reliable channel, once a message $m$ from another group $G$ is \rmd{}ed, every message $m' : m'.ts < m.ts$ from $G$ has also already been \rmd{}ed.
