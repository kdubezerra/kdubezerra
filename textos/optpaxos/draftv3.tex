...
\begin{proof}
From lines \ref{algline:gettime} and \ref{algline:rmlocal}, and the properties of the \rmc{} primitive, we know that all correct processes deliver $m$ and then $m'$, which means that each process $p'$ from $group(p)$ also inserts them into its $propPending$ list in this same order.
