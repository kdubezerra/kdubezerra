...
\textbf{Uniform Agreement}: if a process \cons{}s $m$, then every correct %process eventually also will 
\cons{}s $m$.

\textbf{Uniform Integrity}: for any message $m$, every correct process \cons{}s $m$ at most once, and only if some process executed \cmsend{$m$}.
...
\blankline
\INDENT{\textit{When} receive($m'$)}
  \IF {$G \neq m'.src$ $\wedge$ $(\exists G' \in m'.dst: G' \neq G$ $\wedge$ $G' \neq m'.src)$}
    \STATE $null \leftarrow$ empty message
...
%  \STATE $m.gs \leftarrow k$
  ...
  \ENDIF
  \INDENT{\textbf{for all} $p' \in m.dst \setminus \{G\}$ \textbf{do}}
    \STATE send($p'$, $\{m, \text{`cons'}\}$)  \COMMENT{this message is sent through a FIFO lossless channel}
  \ENDINDENT
\ENDINDENT
