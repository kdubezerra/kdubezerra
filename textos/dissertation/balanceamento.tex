...
%[TODO:por nomes mais simples e elegantes]
...
%[TODO:talvez, compactar essa lista, que está detalhada demais]
...
%[TODO:falar do algoritmo de seleção de regiões do lee]
...
Outro aspecto importante é que cada servidor sempre tem uma região associada. O que pode ocorrer é de uma região ser vazia, ou seja, ela não possui células e o seu servidor não participa da execução do jogo. Isso é útil quando a capacidade total do sistema servidor é muito maior que a carga total do jogo, ou seja, $P_{total} \gg W_{total}$. Neste caso, a introdução de mais servidores aumentaria o overhead de comunicação no sistema, sem melhorar sua qualidade -- salvo quando provesse tolerância a falhas.
...
\begin{algorithmic}[1]
	 ...
	 \FOR{cada região $R$ na $\rlist$}
	 		\STATE $\wtodiv \leftarrow \wtodiv + w_r(R)$
	 		\STATE $\freecap \leftarrow \freecap + p(s(R))$
	 		\STATE $\clist \leftarrow $ lista de celulas de $R$ em ordem decrescente de carga
	 		...
	 		\ENDWHILE
	 \ENDFOR
	 \STATE ordene $\rlist$ em ordem crescente de $u(s(R))$
	 \FOR{cada região $R$ na $\rlist$}
	 		\STATE $\wshare \leftarrow \wtodiv \times \frac{p(s(R))}{\freecap}$
	 		\WHILE{$w_r(R) < \wshare$} \label{alg:ggp:whilefraction}
	 		   \IF{existe alguma célula de $R$ vizinha a uma célula livre}
...
Depois de balancear a carga entre os diferentes servidores na fase 2, cada servidor terá uma taxa de uso de seus recursos semelhante à dos outros. No entanto, pela fase dois não considerar a interação entre as células nem o overhead entre regiões, estas podem ter sido fragmentadas, implicando em muitos pares de células vizinhas que não fazem parte da mesma região. Isso aumenta a probabilidade de dois jogadores, cada um em uma dessas células, interagirem entre si, por estarem próximos da mesma fronteira, o que, como foi mostrado em seções anteriores, causa um desperdício de recursos do sistema servidor.

%[TODO:por o mapeamento de grafo aqui? para de usar a terminologia de grafos... só por no começo um comentário de que célula = vértice etc...]

Para minimizar esse problema, é proposto aqui o uso de uma variante do algoritmo de Kernighan e Lin \cite{kernighan1970ehp}. Como foi mostrado, pode-se mapear as células, regiões e cargas em vértices, partições e pesos, respectivamente, em um grafo. Feito isso, dado um par de partições, o \kl\ busca pares de vértices que, se trocados entre essas partições, diminuirão o corte de aresta.

Sejam $R_A$ e $R_B$ duas regiões e $a$ uma célula tal que $a \in A$. A interação externa da célula $a$ é definida como $E(a) = \sum_{b \in R_B} Int_c(a,b)$ e a interação interna é definida como $I(a) = \sum_{a' \in R_A} Int_c(a,a')$. A célula $a$ está propensa a trocar de região se sua interação com $R_B$ for maior que sua interação com $R_A$, ou seja $D(a) = E(a) - I(a) > 0$. Supondo que exista uma célula $b \in R_B$, tal que $D(a) + D(b) - 2 \times Int_c(a,b) > 0$, a troca irá reduzir o overhead entre as regiões. A expressão $D(a) + D(b) - 2 \times Int_c(a,b)$ é chamada de $gain(a,b)$, pois representa o ganho (redução do corte de aresta) ao serem trocadas $a$ e $b$ de região.

No caso de um ambiente virtual distribuída entre várias regiões, geralmente mais que duas, executa-se o \kl\ para cada par de partições do grafo. Além disso, após cada troca deverá ser mantido o balanceamento entre as regiões -- caso contrário, todas as células poderiam ir para a mesma região, eliminando completamente o overhead. O \kl\ é extensamente conhecido na área de sistemas distribuídos, então não serão dados detalhes aqui. Considera-se apenas que ele retorna um valor de verdadeiro se foi realizada alguma troca e false, se não foi. Ele é executado para todos os pares de regiões até que retorne um valor de falso, indicando que nenhuma troca irá prover ganho adicional.
...
Podemos observar que o \ggp\ é o que tem menor quantidade de walk migrations, justamente por suas regiões serem contíguas. No entanto, pelo fato de não tentar minimizar as transferências de células -- o \ggp\ tem como principal objetivo minimizar o overhead -- a quantidade de still migrations é a maior de todas as suas variantes, sendo melhor apenas que o \bfa, que também foi o pior algoritmo no critério de migrações. A estratégia do \ggpk\ e, mais fortemente, no \ggpf\, de manter o máximo de células possível quando rebalanceando, tem como resultado os menores números de migrações de jogadores dos algoritmos simulados.
