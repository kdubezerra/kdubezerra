...
for {set i 0} {$i < [lindex $argv 0]} {incr i} {
	...
	set aux_pos_mapa [randomnumber 1 [expr [expr [lindex $argv 2] / 2] - 2] ] 
	...
	set aux_pos_mapa [randomnumber [expr [lindex $argv 2] / 2] [expr [lindex $argv 2] -2] ]
	#grupo 1
	set aux_prox $mapa([expr $aux_pos_mapa-1],0)
	set personagens($i,4,onde) [expr $aux_pos_mapa-1]
	set personagens($i,4,time) 0
	set mapa([expr $aux_pos_mapa-1],$aux_prox) $i
	incr mapa([expr $aux_pos_mapa-1],0)

	#grupo 2
	set aux_prox $mapa([expr $aux_pos_mapa+1],0)
	set personagens($i,5,onde) [expr $aux_pos_mapa+1]
	set personagens($i,5,time) 0
	set mapa([expr $aux_pos_mapa+1],$aux_prox) $i
	incr mapa([expr $aux_pos_mapa+1],0)

	#grupo 3 
	set aux_prox $mapa($aux_pos_mapa,0)
	set personagens($i,6,onde) $aux_pos_mapa
	set personagens($i,6,time) 0
	set mapa($aux_pos_mapa,$aux_prox) $i
	incr mapa($aux_pos_mapa,0)
