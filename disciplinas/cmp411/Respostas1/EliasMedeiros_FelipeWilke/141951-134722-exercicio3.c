...
//   sema_num_l
//   sema_saldo
printf("Leitor %d aguardando acesso. \n" , id);
sem_wait(&sema_num_l);
	printf("Leitor %d conseguiu acesso a variavel numero_leitores. \n" , id);
	numero_leitores = numero_leitores+1;
	if(numero_leitores==1){
		sem_wait(&sema_saldo);
		printf("Leitor %d bloqueou escritores. \n" , id);
	}
sem_post(&sema_num_l);
...
sem_wait(&sema_num_l);
	numero_leitores = numero_leitores-1;
   	if(numero_leitores==0){
		sem_post(&sema_saldo);
		printf("Leitor %d liberou escritores. \n" , id);
	}
sem_post(&sema_num_l);
}
