...
int hdr_cosmmus::offset_;

// Mulitmedia Header Class 
static class CosmmusHeaderClass : public PacketHeaderClass {
public:
	CosmmusHeaderClass() : PacketHeaderClass("PacketHeader/Cosmmus",
						    sizeof(hdr_cosmmus)) {
		bind_offset(&hdr_cosmmus::offset_);
	}
} class_cosmmushdr;


// UdpMmAgent OTcl linkage class
static class CosmmusAgentClass : public TclClass {
public:
	UdpMmAgentClass() : TclClass("Agent/UDP/Cosmmus") {}
	TclObject* create(int, const char*const*) {
		return (new CosmmusAgent());
	}
} class_udpcosmmus_agent;


// Constructor (with no arg)
CosmmusAgent::UdpMmAgent() : UdpAgent()
{
	support_mm_ = 0;
	asm_info.seq = -1;
}
