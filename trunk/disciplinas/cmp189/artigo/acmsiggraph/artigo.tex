...
Cada servidor mantém um vetor dos avatares localizados na região por ele administrada, ordenados ao longo do eixo $x$, sendo que cada posição desse vetor contém também um apontador para um índice, formando uma lista, que é ordenada de acordo com a coordenada $y$ dos avatares (Figura \ref{fig:vectorxlisty}). Quando o servidor sobrecarregado dispara o balanceamento, ele percorre a kd-tree a partir do nó que define sua região, subindo um nível na árvore e verificando se a capacidade de seu nó pai é maior ou igual à carga. Enquanto não for encontrado um nó ancestral que tenha uma capacidade maior ou igual à carga, o algoritmo continua subindo na árvore recursivamente, até chegar à raiz. A cada nó visitado, é enviada uma requisição do conjunto de avatares aos servidores representados pelos nós folha das sub-árvores filhas desse nó (Figura \ref{fig:ancestors}. Dispondo desse vetor, e da sua própria lista de avatares, o servidor sobrecarregado pode calcular a carga e a capacidade do seu nó ancestral visitado na kd-tree.

Como os vetores já estão ordenados em ambos os eixos.....
