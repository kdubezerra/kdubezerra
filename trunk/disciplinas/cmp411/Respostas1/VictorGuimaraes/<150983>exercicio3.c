...
//   sema_saldo
	...
	//se for primeiro leitor, entao bloqueia escritores
	if (numero_leitores == 1)
		sem_wait(&sema_saldo);	//primeiro leitor bloqueia o semaforo do saldo	
      sem_post(&sema_num_l);
      //agora pode ler
      printf("Leitor %d leu saldo = %d. \n", id, saldo);
      sleep(1);
    
	 //depois que efetua a leitura, deve verificar se ele é o ultimo leitor. se for o ultimo leitor, deve liberar para os escritores poderem acessar a variavel
     sem_wait(&sema_num_l);
	numero_leitores = numero_leitores -1;
	if (numero_leitores == 0)
		sem_post(&sema_saldo);	//ultimo leitor libera o semaforo do saldo
     sem_post(&sema_num_l);     
