...
\begin{abstract}
%\boldmath
Traditionally, a central server is utilized to provide support to massively multiplayer games, where the number of participants is of the order of tens of thousands. Much work has been done trying to create a fully peer-to-peer model to support this kind of application, but critical questions remain. Examples are: vulnerability to cheating, overload of the upload links of the peers and  In this work, it is proposed the utilization of geographically distributed lower-cost nodes, employing a novell refinement to the area of interest technique, which minimizes the necessary bandwidth to these server nodes. 
