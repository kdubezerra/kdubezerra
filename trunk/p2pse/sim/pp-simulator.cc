...
// 			cout << "OK til here: " << step_interval << "ms interval." << endl;		
	...
	if(argc == 4) {
		if(strcmp(argv[1], "start") == 0) {
			...
			Tcl::instance().evalf("puts \"Simulation started with %ld ms step interval and PPAvatar\"", step_interval, atype);
			return(TCL_OK);
		}
	}
...
void PPSim::setNPlayers(long unsigned n) {
	nPlayers = n;
	cout << "Avatar type is PPAvatar (RoundSmoothAOIAvatar's child class)" << endl;
