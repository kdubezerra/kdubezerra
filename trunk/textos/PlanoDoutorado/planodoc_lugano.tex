...
Another hybrid architecture is that of FreeMMG \cite{cecin2004fsa}. In it, the couple are organized in a peer-to-par in every region of the virtual environment and server mediates the communication between different regions. Again, there is the problem of security: the pairs within a region controlling the simulation that occurs there, can subvert it. It made a probabilistic approach, using a randomly selected pair from another point of the environment to verify the simulation in that region. It is expected that if the couple wish there collude to subvert the game, at least the node inserted there to detect the invalid actions and report to the server. However, besides not completely guarantee security, the problem persists there may be many pairs communicating with many, which may compromise the quality of the game.
