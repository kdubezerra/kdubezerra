...
\item RTS: \emph{real-time strategy}, where each player commands an army, similarly to a chess game, except that the game play is not divided in turns, i.e. it's a real-time game and the player who thinks faster than the others may have an advantage. Nevertheless, the action is usually slower than in an FPS game;

\item RPG: \emph{role-playing game}. In this kind of game, each player has a character who evolves with time, acquiring power and accumulating wealth. These games are characterized by not having a predetermined end of the ``match'', creating a virtual world with persistent state. The player may, then, connect to this world, play for some time with his character, disconnect and, later, reconnect and continue from the point he was when he disconnected for the last time, for his state is stored in the game server.
...
Based on the current state of the art and in previous work -- such as in \cite{bezerra2009lbs, bezerra2008a3, vilanova2008psm, cecin2006pps, cecin2004fsa} --, we can summarize a few requirements that we find critical for the game network support infrastructure to be considered adequate, and which will be dealt with in the doctoral course:
...
\begin{itemize}
	...
	\item \textbf{Availability} - In general, MMOGs require that the players make some kind of payment for account maintenance and, because of that, they tend to be demanding when it comes to availability. Thus, it is desired the system is always available;
	
	\item \textbf{Consistency} - Multiplayer games consist of a virtual environment shared among the players participating in the same match. It is therefore necessary that the environment is represented in the same way on the machines of the various players so that they can interact with each other and with the world in an appropriate manner;
	...
	\item \textbf{Scalability} - Taking into account the number of participants in an MMOG, it is necessary that the system that supports it is able to maintain the quality of service despite the increasing processing and communication load required for this.
...
These solutions will be wrapped together in a middleware, which will provide them as services. These services will be accessible via an API (application programming interface) to allow an easier development of a massively multiplayer game. The game programmer will be able to use these services without needing to have a deep understanding of the distribution model used by the middleware. To the application developer, a high-level interface will be provided, while the middleware will take care of the tasks performed in lower levels. The communication abstraction provided to the game developer may be: publish-subscribe, as in PADRES \cite{fidler2005padres}, distributed shared memory, as in Sinfonia \cite{aguilera2007sinfonia}, message passing or any other that best suits the development of an MMOG.
...
\item Develop a mechanism to deal with entry, exit and crash of server nodes, providing fault tolerance, while trying to keep adjacent, in the overlay network of the server system, the nodes which are geographically close to each other -- the purpose of this is to explore the good connection between them;

%TODO: dividir este item em 2
\item Create an algorithm for balancing the load of the game, maintaining the load on each server proportional to its capacity, while avoiding the splitting of hotspots into different regions, as this would cause a strong dependence between these regions; \cite{bezerra2009lbs};

\item Define a scheme for synchronizing the simulation state among the different server nodes, in order to maintain global consistency;

\item Build a distributed persistence mechanism to store the states of the players and of the virtual world as a whole, so that they are always available, even if some of the server nodes become unexpectedly unavailable;

\item Design techniques and algorithms to reduce inter-server and server-client communication, in order to save resources from the server nodes and reduce delays in the communication between them. As an additional result, the games will have a wider public -- due to the reduction of the bandwidth required to play -- and, also, the requirements for a computer to be part of the server system will be decreased;

\item Create a middleware containing the desired services, along with a high-level API to make them accessible and easy to use to the game developer.
...
In a previous work \cite{bezerra2009lre}, it was set up a preliminary model -- which aimed to partition the virtual environment into regions, each assigned to a single server -- on which a load balancing scheme was defined. The Cosmmus -- Cooperative System for Massive Multiplayer Service -- is a new model to be defined during the doctorate course, which will also use the principle of partitioning the virtual environment of the game into regions, but addressing the most critical problems (as referred in section \ref{researchquestions}), which were left out so far.

Cosmmus will include solutions to the major aspects of MMOGs, such as task distribution, synchronization of the simulation, distributed persistence and so on. Although
this model is not yet defined, there are already some ideas on how it could be and what kind of approach it should use, based on related research. Upon finished, these ideas will be implemented as services in a middleware for developing MMOGs. The following sections describe these ideas, which will be better developed during the doctoral course.
% Como os jogadores interagem!!!
...
As the server nodes are, ideally, of low cost, it is necessary to save their resources as much as possible, but without harming the game quality. For example, it can be made some interest-based filtering, being sent to each player only what he can see, in order to reduce the traffic between clients and servers, and between the servers. Following this same principle, servers from two different regions may have an old version of the state of each other, if there is no interaction between players whose avatars are in different regions. Thus, these servers do not need to update each other so often.
...
Several works have been made in order to perform interest-based message filtering \cite{bezerra2008a3, ahmed2008dai, morgan2005imm, minson2005aim}. In addition to studying which one best fits the proposed model, and evaluate the possible adjustments that are necessary, other ways to save the resources of the server system will be investigated.

\section{High-level application programming interface}

In order to offer to application (game) developers an easy-to-use set of services, a high-level application programming interface must be created for Cosmmus. This set of services will compose the Cosmmus middleware, which will encapsulate all the low level operations regarding the distribution of the game.

There are some options to define this distribution model -- and, also, the abstraction provided to the game developer --, like distributed shared memory, publish/subscribe, message passing and others. Each one of these presents both advantages and disadvantages, but the publish/subscribe model seems to be the most suited for the development of MMOGs, since the entities (avatars and objects) present in the game's virtual world imply state update messages -- which may be published --, each of which may be relevant or not to each player -- who, therefore, may be subscribing or not to these state updates.
...
This section describes the work plan, classes to attend and the intended schedule to perform the activities described in the previous sections. Section \ref{sec:metodologia} describes the activities to execute and the intermediate goals to fulfill the study plan; section \ref{sec:credits} lists the classes to attend and section \ref{sec:lugano} describes shortly the intended international cooperation in Lugano.
...
\item \textbf{Doctoral course requirements}, as imposed, by the postgraduate program in computer science of UFRGS, to the PhD candidate:

\begin{enumerate}
	\item Validation of the 24 credits obtained during the Master;
	\item Obtainment of the remaining credits required for the doctorate course;
	\item Realization of teaching activity;
	\item Presentation of the research stated during the academic week of UFRGS;
	\item Obtaining of approval in a broad knowledge examination;
	\item Presentation of the thesis proposal.
\end{enumerate}

\item \textbf{Literature review}: aims to update and expand the domain of the state of the art related to the distribution of MMOGs:
%TODO: por em emph palavras estrangeiras ao longo do plano, mas so na primeira aparição do termo
\begin{enumerate}
	\item Study overlay networks formation technology and check its compatibility with the intended type of management of entry, exit and crash of server nodes;
	\item Search for papers offering techniques aimed at allocating the servers on the network overlay, taking into account the quality of communication between them (i.e. servers geographically close -- and with a good connection between them -- tend to be adjacent in the overlay network);
	\item Analysis of load balancing algorithms, with regard to its applicability on a distributed server for MMOGs, in order to determine what adaptations are needed to the context of games;
	\item Search works that are intended for the discovery and management of hotspots, so that their presence does not degrade the quality of service of the game;
	\item Review of techniques to synchronize the state of the game among different servers dynamically, without generating excessive traffic on the server system;
	\item Search mechanisms for distributed, consistent and fault-tolerant persistent, which can be used to store the states of the players and of the virtual environment;
	\item Study on existing techniques to reduce the traffic between clients and servers and between servers, so as to reduce costs without harming the game quality.
\end{enumerate} 

\item \textbf{Model definition}: aims to define the components necessary for the architecture to be able to provide the desired service:

\begin{enumerate}
	\item Definition of the kind of topology that the overlay network should have and what technology will be used/adapted for the management of this network;
	\item Definition of criteria used to perform load balancing and how it will be done;
	\item Definition of the synchronization scheme among the server nodes;
	\item Definition of the persistence model used to store in a distributed and fault-tolerant manner the state of the virtual world and the players;
	\item Integration of the model defined with techniques for optimizing the use of bandwidth and reducing delay in communications;
	\item International cooperation with another research group, via sandwich or co-tuition (more details in section \ref{sec:lugano}).
\end{enumerate}

\item \textbf{Implementation of the model}, which is aimed to create a prototype that can be used both in simulations and in actual implementations of the proposed model:

\begin{enumerate}
	\item Implementation of the layer for managing the network connections between node servers and between servers and clients;
	\item Implementation of the module responsible for load distribution and hotspots management;
	\item Implementation layer of simulation synchronization and state persistence;
	\item Creation and integration of module responsible for monitoring the node's resource usage and adapting the quality of the game to the capacity of each server.
\end{enumerate}

\item \textbf{Simulation and tests}, whose objective is to validate the proposed model and verify whether the defined objectives have been attained:

\begin{enumerate}

	\item Analysis and comparison of network and large-scale systems simulators, one of which shall be chosen to perform the simulations of the proposed model to support distributed MMOGs;
	\item Adaptation of the prototype to run on the chosen simulator;
	\item Performing of tests with respect to: quality of communication between adjacent nodes; efficiency of load balancing and hotspots detection; consistency of the simulation among different server nodes; availability of the game and of the states of the players, even in the presence of faults;
	\item Test the prototype on a real large scale system, such as Grid5000 \cite{bolze2006gls} and analyze the results.
\end{enumerate}

\item \textbf{Thesis' writing}: documentation of the work as a PhD thesis, and publish it, among with conference and journal papers.

\begin{enumerate}
	\item Thesis writing;
	\item Writing of articles for national (SBGames) and international (\mbox{DS-RT}, NetGames) publication, besides international journals, such as \emph{Multimedia Tools and Applications}, for example.
\end{enumerate}
...
\section{Courses to attend and credits to obtain}
\label{sec:credits}

In order to fulfill the requirements of the doctorate course in UFRGS, it is intended to:

\begin{enumerate}
	\item Validate the 24 credits obtained during the Master course in Computer Science, in UFRGS. The attended courses are listed in table \ref{tab:revalidar};
	\item Obtain 12 additional credits, by attending the courses CMP134, CMP189 and CMP112;
	\item Obtain 3 more credits, corresponding to a written work and the realization of teaching activity (Table \ref{tab:creditos}).
\end{enumerate}


\begin{table}[h]
\begin{center}
\caption{Credits, from Master, to validate}
\label{tab:revalidar}
\begin{tabular}{|c||c|c|c|c|c|c|c|c|}
\hline
Code   	 & Subject & Credits & Grade \\ \hline \hline
CMP139   & Fault-tolerance in distributed systems & 4 & A \\ \hline
CMP255   & Parallel programming for HPC & 4 & A \\ \hline
CMP167   & Distributed objects programming & 4 & A \\ \hline
CMP256   & Design of parallel programs for HPC & 4 & A \\ \hline
CMP157   & Parallel and distributed programming & 4 & A \\ \hline
CMP250   & Conceptual aspects of fault-tolerance & 1 & B \\ \hline
CMP401   & Written work I & 2 & A \\ \hline
CMP410   & Teaching activity I & 1 & A \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\begin{table}[h]
\begin{center}
\caption{Credits obtained during the doctorate course}
\label{tab:creditos}
\begin{tabular}{|c||c|c|c|c|c|c|c|c|}
\hline
Code   & Subject & Credits \\ \hline \hline
CMP112   & Distributed information systems & 4 \\ \hline
CMP134   & Introduction to parallel and distributed processing & 4 \\ \hline
CMP189   & Geometric algorithms & 4 \\ \hline
%CMP260   & Redes P2P: características, protocolos e aplicações & 4        \\ \hline
CMP402 & Written work II & 2 \\ \hline
CMP411   & Teaching activity II & 1 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
Table \ref{tab:cronograma} presents the schedule for the activities depicted in the previous sections, as listed in section \ref{sec:metodologia}.
...
Before entering the Master course, the candidate has already worked with research and development, in the independent group \emph{indigente} \cite{indigente}, being one of the mentors and developers of the game engine InGE \cite{rocha2007dmm} and the sole programmer of its network module. Next, he was one of the developers of the game \emph{We Are The Champignons}, which obtained the 3rd prize in a national indie games contest -- the ``Game Festival'', part of the 5th Brazilian Symposium on Computer Games and Digital Entertainment, in 2006.

One of the objectives of the doctoral course is to continue the research started during the Master's course, during which a technique for bandwidth usage optimization have been proposed: the A$^3$ algorithm \cite{bezerra2008a3}, which consists of a fine-tuned interest management, based on different relevance levels for each pair of entities in the game. Another work done during the Master's was the definition of a distributed server load balancing scheme \cite{bezerra2009lbs} which takes into consideration several points that have been considered critical: first, the distributed MMOG server system is not necessarily homogeneous, requiring the load balancing scheme to consider the different amounts of resources; second, the players may move their avatars to certain positions with a higher probability, leading to the formation of densely populated areas (hotspots) in the virtual environment; and, finally, the algorithms proposed tried to minimize the inter-server communication by using a graph partitioning approach.

Additionally, the candidate participated of the P2PSE project \cite{vilanova2008psm}, as one of the developers responsible for the multi-server support in the library. The programming language used was C++. Besides, he set up and executed the network tests with the ns-2 simulator \cite{mccanne:nsn}, whose results have been considered satisfying.
