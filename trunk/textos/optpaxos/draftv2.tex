...
%  \STATE $m.gs \leftarrow k$
  ...
  \STATE $propPending \leftarrow propPending \setminus (\{m\} \cup \{m' : m'.ts < m.ts\})$
  \STATE $nextProp \leftarrow k + 1$
...
The problem with addressing liveness this way is that, in the worst case, $G'$ has decided a message $m$ and has just received the last barrier $b$ from $G$, such that $b$ is less than $m.ts$ by an infinitesimal difference. This would mean that, if $G$ has no messages to send to $G'$, $G'$ will have to wait at least for $barrierThreshold + T(consensus)$, where $T(consensus)$ is the time needed for a value to be decided after it has been proposed in some consensus instance -- assuming that such value is going to be decided at all in that instance. Because of that, the conservative delivery of $m$ will be delayed by $w(p) + barrierThreshold + 2T(consensus)$, where $w(p)$ is the wait window of the process $p \in G$, and $T(consensus)$ is counted twice because there are two consensus proposals -- first in $G'$ regarding $m$, and then in $G$ regarding the empty message.
...
\caption{\cmsend{m} -- Algorithm executed by every process $p$ from group $G$}
