...
\INDENT {Initialization}
  \STATE $groupClock \leftarrow 0$, $nextProp \leftarrow 0$, $decided \leftarrow \emptyset$, $propPending \leftarrow \emptyset$,\\
  $optPending \leftarrow \emptyset$, $consPending \leftarrow \emptyset$
...
\blankline\
\INDENT{\textit{To send a message $m$}}
...
\blankline\
\INDENT{\textit{When $\exists m \in optPending :$\\
$getTime() > m.ts + w(p)$ $\wedge\ \nexists m' \in optPending: m'.ts < m.ts$}}
...
\blankline\
\INDENT{\textit{When $(\exists m \in propPending : \nexists m' \in propPending: m'.ts < m.ts)$\\
$\wedge$ $nextProp = groupClock$}}
%  \STATE $propPending \leftarrow propPending \setminus \{m\}$
%  \IF {$m \notin decided \wedge \nexists m' \in decided: m'.ts > m.ts$}
    \STATE $k \leftarrow groupClock$
    \STATE $nextProp \leftarrow groupClock + 1$
    \STATE Propose($k$,$m$)
%  \ENDIF
\ENDINDENT
...
%  \STATE $m.gs \leftarrow k$
  ...
  \STATE $nextProp \leftarrow groupClock \leftarrow k + 1$
