...
More formally, let $\delta(p',p)$ be the maximum time for a message from $p'$ to arrive at $p$. Also, let $\epsilon(p',p)$ be the deviation between the wallclocks of $p$ and $p'$. The value of $w(p)$ is estimated as the maximum value of $(\delta(p',p) + \epsilon(p',p))$ for every $p'$ in $G = group(p)$ or in $sendersTo(G)$.
...
\INDENT {Initialization}
  \INDENT{\textbf{for all} $G' : G' \in sendersTo(G)$ \textbf{do}}
...
\blankline\
\INDENT {Initialization}
  \INDENT{\textbf{for all} $G' : G' \in receiversFrom(G)$ \textbf{do}}
    \STATE $lastBarrierSent(G') = -\infty$
  \ENDINDENT
\ENDINDENT
\blankline\
\INDENT{\textit{When} $Learn(\{m\})$ $\wedge$ $m.src = G$}
  \INDENT{\textbf{for all} $G' : G' \in m.dst$ \textbf{do}}
...
\INDENT{\textit{When $\exists G' \in receiversFrom(G):$\\$getTime() - lastBarrierSent(G') > barrierThreshold$}}
  ...
  \COMMENT{saving that nothing was sent until em.ts}
\ENDINDENT
\blankline\
