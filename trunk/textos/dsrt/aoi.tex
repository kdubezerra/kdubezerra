...
No que se refere a gerenciamento de interesse, trabalhos como \cite{morse1996iml, morgan2005imm, rak1996egb} podem ser citados. Em RAK, é proposto um esquema de gerenciamento de interesse baseado em células de uma grade que compõe o ambiente virtual. A cada célula está associado um grupo de multicast. Cada participante da simulação se subscreve então no grupo de multicast da célula onde ele se encontra, assim como das células alcançadas por uma área definida ao seu retor. Para enviar atualizações de estado, cada um envia para o grupo de multicast da região onde se encontra. O problema desta abordagem é que não se pode assumir que haverá suporte a multicast sobre a Internet, já que não é amplamente estabelecido. Além disso, uma área de interesse formada por células fixas envolverá um número maior de objetos do que seria realmente necessário do que ocorreria se fosse utilizada uma área completamente dinâmica.

Em Zou, são considerados esquemas de gerenciamento de interesse baseados em grupos de multicast. É feita uma comparação entre a formação de grupos baseado em célula, onde cada um está associado a uma célula estática do ambiente virtual, e agrupamento baseado em entidades, onde para cada entidade existe um grupo multicast associado. Verificou-se que há uma compensação (tradeoff) entre o custo das mensagens de controle dos grupos de multicast e o custo das mensagens de atualização de estado propriamente ditas.

Um esquema de gerenciamento de interesse que utiliza um middleware orientado a mensagens é apresentado em MORGAN. Dentre outros aspectos, este esquema faz uma predição do que será o interesse de determinado participante no futuro, baseado na posição e vetor velocidade do mesmo no ambiente virtual. Dessa forma, cada um começa a receber atualizações de estado de entidades que não estão ao seu alcance ainda, mas que provavelmente estarão em um futuro próximo.

%por morse por último e justificar baseado nos parâmetros criados por ela.

Em KLMORSE, é feito um apanhado de sistemas que utilizavam esta técnica, salientando quais critérios eram utilizados por cada um. É apresentada então uma taxonomia de tais sistemas, classificando-os de acordo com: modelo de comunicação, foco da filtragem e domínios de responsabilidade. O modelo pode ser \textit{unicast}, \textit{multicast} ou \textit{broadcast}. %Como supõe-se aqui um sistema distribuído sobre a Internet, sobre a qual o suporte a \textit{multicast} não está ainda amplamente estabelecido, considera-se apenas o modelo de comunicação \textit{unicast}.
O foco da filtragem refere-se a onde está sendo feita esta filtragem, se é no nível de rede (\textit{extrínseca}), com base em endereços IP, ou no nível da aplicação com base em parâmetros mais específicos (\textit{intrínseca}). Por fim, o domínio de responsabilidade atribuída a um gerenciador de interesse, que verifica para quem cada estado é relevante, pode ser \textit{dinâmico} - os domínios sobre os quais cada participante pode ter interesse é variável (uma área ao redor de cada participante) - ou \textit{estático} - como uma grade de células fixas, por exemplo.
